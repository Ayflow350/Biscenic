...
// Generic array field item for simple string lists
const stringArrayField = z.array(z.string().min(1)).default([]);

// Option objects
const materialOption = z.object({
  name: z.string().min(1),
  price: z.coerce.number().positive(),
  status: z.nativeEnum(AvailabilityStatus),
});

const finishOption = z.object({
  colorName: z.string().min(1),
  hexCode: z.string().optional(),
  status: z.nativeEnum(AvailabilityStatus),
});

const editionOption = z.object({
  name: z.string().min(1),
  description: z.string().optional(),
});

const luxuryFeatureOption = z.object({
...
// Field array names for useFieldArray
